...
ORYX.Utils = {
	...
	adjustGradient: function(gradient, reference){
		...
		if (ORYX.CONFIG.DISABLE_GRADIENT && gradient){
			...
			$A(gradient.getElementsByTagName("stop")).peach(function(stop){
...
var Kickstart = {
	...
	kick: function() {
		...
		if(!Kickstart.started) {
			...
			Kickstart.callbacks.peach(function(callback){
...
Kickstart.register(Kickstart.load)
...
var ERDF = {
	...
	_parseFromTag: function(node, subject, depth) {
		...
		if(children) $A(children).peach(function(_node) {
	...
	_parseTriplesFrom: function(subjectType, subject, properties,
		...
		properties.toLowerCase().split(' ').peach( function(property) {
...
var DataManager = {
	...
	__forceExistance: function(shape) {
		...
		if(!$Q(shape.resourceId)) {
			
			if(!$$Q('.' + PROCESSDATA_REF)[0])
			...
			var resource = $Q(shape.resourceId)
			var children = $A(resource.childNodes)
			children.peach( function(child) {
	...
	__persistDOM: function(facade) {
		...
		result += DataManager.serialize(
				$Q(ERDF.__stripHashes(facade.getCanvas().resourceId)), true);
				
		shapes.peach( function(shape) {
			
			DataManager.__persistShape(shape);
			result += DataManager.serialize(
				$Q(ERDF.__stripHashes(shape.resourceId)), true);
		});
	...
	__renderCanvas: function(facade) {
		...
		stencilSets.values().peach(function(stencilset) {
			DataManager.addTriple( new ERDF.Triple(
				shapeResource,
			...
			stencilset.extensions().keys().peach(function(extension) {
				DataManager.addTriple( new ERDF.Triple(
					shapeResource,
	...
	__provideId: function() {
		...
		while($Q(RESOURCE_ID_PREFIX+DataManager.__counter))
	...
	__syncglobal: function(facade) {
		...
		shapes.select( function(shape) {
			...
			return !($Q(shape.resourceId));

		}).peach( function(shape) {
	...
	serialize: function(node, preserveNamespace) {
		...
		if (node.nodeType == node.ELEMENT_NODE) {
			...
			attributes.peach(function(attribute) {
...
ResourceManager = {
	...
	__dispatchEvent: function(event) {
		...
		ResourceManager.__listeners.values().peach(function(controller) {
		...
		if(	(collectionUrls.length == 1) &&
			...
			ResourceManager.__request(
				...
				function() {
					...
					$$Q('.' + PROCESSDATA_REF)[0].appendChild(div.cloneNode(true));
...
//		}
		...
		this.save = function(synchronize) {
			var _url = this.__url;
			var _id = this.__id;
			data = DataManager.serialize($Q(_id));
			ResourceManager.__request(
				'PUT', _url, data,
				function() { ResourceManager.__resourceActionSucceeded(
...
ORYX.CONFIG.PLUGINS_CONFIG =				"libs/editor/plugins.xml";
...
ORYX = Object.extend(ORYX, {
	...
	Log: {
		...
		__log: function(prefix, messageParts) {
			...
			ORYX.Log.__appenders.peach(function(appender) {
	...
	_loadPlugins: function() {
		...
		new Ajax.Request(source, {
			...
			onSuccess: function(result) {
				...
				preferences.peach( function(p) {

					var props = $A(p.childNodes);
					props.peach( function(prop) {
						var property = new Hash(); 
						
						// get all attributes from the node and set to global properties
						var attributes = $A(prop.attributes)
						attributes.peach(function(attr){property[attr.nodeName] = attr.nodeValue});				
				...
				$A(plugin).peach( function(node) {
					...
					var properties = [];
					$A(propertyNodes).peach(function(prop) {
						var property = new Hash(); 
					...
					var requires;
					$A(requireNodes).peach(function(req) {			
					...
					var notUsesIn;
					$A(notUsesInNodes).peach(function(not) {			
...
ORYX.Core.SVG.SVGMarker = Clazz.extend({
	...
	_getSVGShapes: function(svgElement) {
		if(svgElement.hasChildNodes) {
			var svgShapes = [];
			var me = this;
			$A(svgElement.childNodes).peach(function(svgChild) {
				try {
...
//			this.svgShapes.peach(function(svgShape) {
//
//			});
//		}
...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	construct: function(options) {
		...
		if(alignValues) {
			...
			alignValues.peach((function(alignValue) {
				switch (alignValue) {
					case 'top':
					case 'middle':
					case 'bottom':
			...
			if (this.isVisible) {
				...
				if (this.node.ownerDocument) {
					...
					if (this.fitToElemId || this._textHasChanged){
						...
						textLines.peach((function(textLine, index){
	...
	_positionText: function() {
		try {
			...
			invalidTSpans.peach(function(tspan) {
		...
		// Raise change event
		(this.changeCallbacks||[]).peach(function(fn){
			fn.apply(fn);
		})
...
ORYX.Core.StencilSet.Stencil = {
	...
	construct: function(jsonStencil, namespace, source, stencilSet, propertyPackages, defaultPosition) {
		...
		this._jsonStencil.roles.peach((function(role, index) {
	...
	postProcessProperties: function() {
		...
		if(this._jsonStencil.propertyPackages && this._jsonStencil.propertyPackages instanceof Array) {
			...
			this._jsonStencil.propertyPackages.peach((function(ppId) {
				var pp = this._propertyPackages[ppId];
				
				if(pp) {
					pp.peach((function(prop){
		...
		if(this._jsonStencil.properties && this._jsonStencil.properties instanceof Array) {
			this._jsonStencil.properties.peach((function(prop) {
				var oProp = new ORYX.Core.StencilSet.Property(prop, this._namespace, this);
				this._properties[oProp.prefix() + "-" + oProp.id()] = oProp;
			}).bind(this));
		}
    ...
    construct: function(jsonProp, namespace, stencil){
        ...
        if (jsonProp.type === ORYX.CONFIG.TYPE_CHOICE) {
            if (jsonProp.items && jsonProp.items instanceof Array) {
                jsonProp.items.peach((function(jsonItem){
            ...
            if (jsonProp.type === ORYX.CONFIG.TYPE_COMPLEX || jsonProp.type == ORYX.CONFIG.TYPE_MULTIPLECOMPLEX) {
                if (jsonProp.complexItems && jsonProp.complexItems instanceof Array) {
                    jsonProp.complexItems.peach((function(jsonComplexItem){
	...
	construct: function(jsonItem, namespace, property) {
		...
		if(jsonItem.type === ORYX.CONFIG.TYPE_CHOICE) {
			if(jsonItem.items && jsonItem.items instanceof Array) {
				jsonItem.items.peach((function(item) {
			...
			if(jsonItem.complexItems && jsonItem.complexItems instanceof Array) {
				jsonItem.complexItems.peach((function(complexItem) {
...
ORYX.Core.StencilSet.Rules = {
	...
	initializeRules: function(stencilSet) {
		...
		if (existingSS) {
			...
			stencilSet.extensions().values().peach(function(extension) {
				if(extension.rules) {
			...
			if (jsonRules.connectionRules) {
				jsonRules.connectionRules.peach((function(rules){
					...
					rules.connects.peach((function(connect){
						var toRoles = [];
						if (connect.to) {
							if (!(connect.to instanceof Array)) {
								connect.to = [connect.to];
							}
							connect.to.peach((function(to){
			...
			if (jsonRules.cardinalityRules) {
				jsonRules.cardinalityRules.peach((function(rules){
					var cardrKey;
					if (this._isRoleOfOtherNamespace(rules.role)) {
						cardrKey = rules.role;
					}
					...
					if (rules.outgoingEdges) {
						rules.outgoingEdges.peach((function(rule){
					...
					if (rules.incomingEdges) {
						rules.incomingEdges.peach((function(rule){
							if (this._isRoleOfOtherNamespace(rule.role)) {
								ie[rule.role] = rule;
							}
							else {
			...
			if (jsonRules.containmentRules) {
				jsonRules.containmentRules.peach((function(rules){
					var conrKey;
					if (this._isRoleOfOtherNamespace(rules.role)) {
						conrKey = rules.role;
					}
					...
					(rules.contains||[]).peach((function(containRole){
			...
			if (jsonRules.morphingRules) {
				jsonRules.morphingRules.peach((function(rules){
					var morphrKey;
					if (this._isRoleOfOtherNamespace(rules.role)) {
					...
					rules.baseMorphs.peach((function(baseMorphStencilId){
			...
			if (jsonRules.layoutRules) {
				...
				jsonRules.layoutRules.peach(function(rules){
					var layoutKey;
					if (this._isRoleOfOtherNamespace(rules.role)) {
	...
	outgoingEdgeStencils: function(args) {
		...
		this._stencils.peach((function(stencil) {
...
//		}
	...
	morphStencils: function(args) {
		...
		var _morphStencils = [];
		args.stencil.roles().peach(function(role) {
			this._cacheMorph(role).peach(function(stencil) {
	...
	baseMorphs: function() {
		var _baseMorphs = [];
		this._morphingRules.peach(function(pair) {
			pair.value.peach(function(baseMorph) {
	...
	getLayoutingRules : function(shape, edgeShape){
		...
		// For each role
		shape.getStencil().roles().peach(function(role) {
	...
	_getMaximumOccurrence: function(parent, child) {
		var max;
		child.roles().peach((function(role) {
	...
	_getMaximumNumberOfOutgoingEdge: function(args) {
		...
		var max;
		args.sourceStencil.roles().peach((function(role) {
			var cardRule = this._cardinalityRules[role];

			if(cardRule && cardRule.outgoingEdges) {
				args.edgeStencil.roles().peach(function(edgeRole) {
	...
	_getMaximumNumberOfIncomingEdge: function(args) {
		...
		var max;
		args.targetStencil.roles().peach((function(role) {
			var cardRule = this._cardinalityRules[role];
	...
	_getConnectionRulesOfEdgeStencil: function(edgeStencil) {
		...
		edgeStencil.roles().peach((function(role) {
			if(this._connectionRules[role]) {
				this._connectionRules[role].peach(function(cr) {
					if(edgeRules[cr.key]) {
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			new Ajax.Request(backend.flowService + "/api/biz/event/chart/stencil-sets?version=" + Date.now(), {
	            asynchronous: false,
	            method: 'get',
		...
		try {
			...
			if(jsonExtension["extends"] == this.namespace()) {
				...
				if(jsonExtension.stencils) {
					$A(jsonExtension.stencils).peach(function(stencil) {
						defaultPosition++;
				...
				if (jsonExtension.properties) {
					...
					stencils.peach(function(stencil){
						var roles = stencil.roles();
						
						jsonExtension.properties.peach(function(prop){
							prop.roles.any(function(role){
								role = jsonExtension["extends"] + role;
								if (roles.member(role)) {
									prop.properties.peach(function(property){
				...
				if(jsonExtension.removeproperties) {
					jsonExtension.removeproperties.peach(function(remprop) {
						var stencil = this.stencil(jsonExtension["extends"] + remprop.stencil);
						if(stencil) {
							remprop.properties.peach(function(propId) {
				...
				if(jsonExtension.removestencils) {
					$A(jsonExtension.removestencils).peach(function(remstencil) {
    ...
    __handleStencilset: function(response){
        ...
        try {
        	...
        	this._jsonObject = JSON.parse(data.data);
    ...
    _init: function(response){
		...
		if(this._jsonObject.propertyPackages) {
			$A(this._jsonObject.propertyPackages).peach((function(pp) {
        ...
        // init each stencil
        $A(this._jsonObject.stencils).peach((function(stencil){
        	defaultPosition++;
...
ORYX.Core.StencilSet.stencilSets = function(editorId) {
	...
	if(stencilSetNSs) {
		stencilSetNSs.peach(function(stencilSetNS) {
...
ORYX.Core.Bounds = {
	...
	_changed: function(sizeChanged) {
		if(!this.suspendChange) {
			this._changedCallbacks.peach(function(callback) {
...
ORYX.Core.UIObject = {
	...
	update: function() {
		if(this.isChanged) {
			...
			this.children.peach(function(value) {
				value.update();
			});
		}
	},
	...
	hide: function() {
		...
		this.isVisible = false;
		this.children.peach(function(uiObj) {
			uiObj.hide();	
		});
	},
...
/** @lends ORYX.Core.AbstractShape.prototype */
{
	...
	construct: function(options, stencil, facade) {
		...
		this._stencil.properties().peach((function(property) {
		...
		if (stencil._jsonStencil.superId) {
			stencil.properties().peach((function(property) {
				var key = property.prefix() + "-" + property.id();
	...
	getChildShapes: function(deep, iterator) {
		...
		this.children.peach(function(uiObject) {
	...
	getAbstractShapesAtPosition: function() {
		...
		if(this.isPointIncluded(x, y)) {
			...
			[childNodes, childEdges].peach(function(ne){
				var nodesAtPosition = new Hash();
				
				ne.peach(function(node) {
				...
				nodesAtPosition.keys().sort().peach(function(key) {
	...
	serialize: function() {
		...
		this.hiddenProperties.peach(function(prop){
			serializedObject.push({name: prop.key.replace("oryx-", ""), prefix: "oryx", value: prop.value, type: 'literal'});
		}.bind(this));
		
		// Add all properties
		this.getStencil().properties().peach((function(property){
...
ORYX.Core.AbstractShape.JSONHelper = {
     ...
     eachChild: function(iterator, deep, modify){
         ...
         this.childShapes.peach(function(shape){
...
ORYX.Core.Canvas = ORYX.Core.AbstractShape.extend({
	...
	update: function() {
		...
		this.nodes.peach(function(node) {
		...
		if(layoutEvents) {
			layoutEvents.peach(function(event) {
		...
		/*this.children.peach(function(child) {
			child._update();
		});*/
	},
	...
	getChildNodes: function(deep, iterator) {
		if(!deep && !iterator) {
			...
			var result = [];
			this.nodes.peach(function(uiObject) {
				if(iterator) {
					iterator(uiObject);
				}
				result.push(uiObject);
...
/*	getChildEdges: function(iterator) {
		if(iterator) {
			this.edges.peach(function(edge) {
				iterator(edge);
			});
		}
...
*/	
    ...
    addShapeObjects: function(shapeObjects, eventHandler){
        ...
        var addChildShapesRecursively = function(shape){
            ...
            shape.childShapes.peach(function(childShape){
        ...
        shapes.peach(
            function(shape){
                ...
                for(field in shape.json.properties){
                  ...
                  // Outgoings
                  shape.json.outgoing.peach(function(out){
                    properties.push({
                      prefix: 'raziel',
                      name: 'outgoing',
	...
	getSVGRepresentation: function(escapeText) {
		...
		var x1, y1, x2, y2;
		this.getChildShapes(true).peach(function(shape) {
		...
		if(escapeText) {
			$A(svgClone.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG, 'tspan')).peach(function(elem) {
				elem.textContent = elem.textContent.escapeHTML();
			});
			
			$A(svgClone.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG, 'text')).peach(function(elem) {
		...
		$A(svgClone.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG, 'image')).peach(function(elem) {
		...
		// escape all links
		$A(svgClone.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG, 'a')).peach(function(elem) {
	...
	*/
	...
	/*cleanUp: function(parent) {
		...
		parent.getChildShapes().peach(function(shape){
...
ORYX.Editor = {
	...
	getAvailablePlugins: function(){
		...
		curAvailablePlugins.peach(function(plugin){
				...
				ORYX.Log.debug("Initializing plugin '%0'", match.name);
					...
					try {
						...
						var className 	= eval(match.name);
							...
							this.loadedPlugins.peach(function(loaded){
	...
	loadPlugins: function() {
		...
		if( ORYX.MashupAPI && ORYX.MashupAPI.loadablePlugins && ORYX.MashupAPI.loadablePlugins instanceof Array ){
			...
			ORYX.MashupAPI.loadablePlugins.peach(function( className ){
		...
		ORYX.availablePlugins.peach(function(value) {
		...
		newPlugins.peach(function(value) {
			// If there is an GUI-Plugin, they get all Plugins-Offer-Meta-Data
	...
	*/
		...
		if(jsonObject.stencilset.namespace && jsonObject.stencilset.namespace !== this.getCanvas().getStencil().stencilSet().namespace()) {
			...
			execute: function(){
				...
				if (!this.shapes) {
					...
					this.shapes.peach(function(shape) {
					...
					this.connections.peach(function(con) {
				...
				rollback: function(){
					...
					/*this.parents.values().uniq().peach(function(parent) {
    ...
    renewResourceIds: function(jsonObject){
        ...
        resourceIds.peach(function(oldResourceId){
    ...
    loadSSExtensions: function(ss_extension_namespaces){
        ...
        ss_extension_namespaces.peach(function(ss_extension_namespace){
	...
	*/
	...
	registerPluginsOnKeyEvents: function() {
		this.pluginsData.peach(function(pluginData) {
			
			if(pluginData.keyCodes) {
				
				pluginData.keyCodes.peach(function(keyComb) {
	...
	*/
	...
	deleteShape: function(shape) {
		...
		shape.getOutgoingShapes().peach(function(os) {
		...
		shape.getIncomingShapes().peach(function(is) {
		...
		shape.getDockers().peach(function(docker) {
			docker.setDockedShape(undefined);
		});
	},
	...
	*/
	_executeEventImmediately: function(eventObj) {
		if(this.DOMEventListeners.keys().member(eventObj.event.type)) {
			this.DOMEventListeners[eventObj.event.type].peach((function(value) {
...
ORYX.Editor.Cookie = {
	...
	start: function( interval ){
		...
		this.pe = new PeriodicalExecuter( function(){
			...
			if( currentString != document.cookie ){
				...
				this.callbacks.peach(function(callback){ callback(this.getParams()) }.bind(this));
			}
	...
	getParams: function(){
		...
		p.split("; ").peach(function(param){ res[param.split("=")[0]] = param.split("=")[1];});
...
new function(){
	...
	ORYX.Core.MoveDockersCommand = ORYX.Core.Command.extend({
		...
		execute: function(){
			...
			this.dockers.values().peach(function(docker){
		...
		executeAgain: function(){
			this.changes.values().peach(function(change){
				// Reset the dockers
				this.removeAllDocker(change.edge);
				change.dockerPositions.peach(function(pos, i){	
		...
		rollback: function(){
			this.changes.values().peach(function(change){
				// Reset the dockers
				this.removeAllDocker(change.edge);
				change.oldDockerPositions.peach(function(pos, i){	
		...
		removeAllDocker: function(edge){
			edge.dockers.slice(1, edge.dockers.length-1).peach(function(docker){
...
ORYX.Core.Shape = {
	...
	refresh: function() {
		...
		if(this.node.ownerDocument) {
			...
			var me = this;
			this.propertiesChanged.peach((function(propChanged) {
				if(propChanged.value) {
					...
					if (property != undefined) {
						...
						if(property.type() == ORYX.CONFIG.TYPE_CHOICE) {
							...
							property.refToView().peach((function(ref) {
							...
							property.items().peach((function(item) {
								item.refToView().peach((function(itemRef) {
									if(itemRef == "") { return; }
										...
										case ORYX.CONFIG.TYPE_COLOR:
											if(property.fill()) {
												if (svgElem.tagName.toLowerCase() === "stop"){
													...
													if (svgElem.parentNode.tagName.toLowerCase() === "radialgradient"){
														$A(svgElem.parentNode.getElementsByTagName('stop')).peach(function(stop){
			...
			//update labels
			this._labels.values().peach(function(label) {
				label.update();
			});
		}
	},
	...
	serialize: function() {
		...
		this.getOutgoingShapes().peach((function(followingShape){
	...
	deserialize: function(serialize, json){
		...
		if (json && json.labels instanceof Array){
			json.labels.peach(function(slabel){
	...
	toJSON: function(){
		...
		this._labels.peach(function(obj){
...
ORYX.Core.Node = {
    ...
    _update: function(){
		...
		if (this.isChanged) {
			...
			if (this.isResized) {
				...
				this._svgShapes.peach(function(svgShape){
					//adjust width
				...
				this.magnets.peach(function(magnet){
				...
				this.getLabels().peach(function(label){
		...
		if (this.dockers.length > 0&&!this.dockers.first().getDockedShape()) {
			this.dockers.peach(function(docker){
				docker.bounds.centerMoveTo(this.bounds.center())
			}.bind(this))
		}
		
		/*this.incoming.peach((function(edge) {
			if(!(this.dockers[0] && this.dockers[0].getDockedShape() instanceof ORYX.Core.Node))
				edge._update(true);
		}).bind(this));
		
		this.outgoing.peach((function(edge) {
			if(!(this.dockers[0] && this.dockers[0].getDockedShape() instanceof ORYX.Core.Node))
				edge._update(true);
		}).bind(this)); */
    },
    ...
    _init: function(svgDocument){
        ...
        if (magnets && magnets.length > 0) {
            ...
            var me = this;
            magnets.peach(function(magnetElem){
        ...
        $A(textElems).peach((function(textElem){
...
ORYX.Core.Edge = {
	...
	refreshAttachedNodes: function() {
		this.attachedNodePositionData.values().peach(function(nodeData) {
    ...
    refresh: function(){
        ...
        var lastPoint;
        this._paths.peach((function(path, index){
	...
	*/
    ...
    alignDockers: function(){
        ...
        this.dockers.peach((function(docker, index){
	...
	handleChildShapesAfterAddDocker: function(docker) {
		...
		segmentElements.peach(function(nodePositionData) {
	...
	handleChildShapesAfterRemoveDocker: function(docker) {
		...
		this.attachedNodePositionData.peach(function(nodePositionData) {
	...
	removeUnusedDockers:function(){
		...
		this.dockers.peach(function(docker, i){
    ...
    _init: function(svgDocument){
        ...
        if (defs.length > 0) {
            ...
            var marker;
            var me = this;
            markerElements.peach(function(markerElement){
                try {
                    ...
                    var label;
                    textElements.peach(function(textElement){
        ...
        $A(g.childNodes).peach((function(path, index){
            if (ORYX.Editor.checkClassType(path, SVGPathElement)) {
				...
				for (var i = 0; i < handler.points.length; i += 2) {
					//handler.points.peach((function(point, pIndex){
        ...
        this._paths.reverse();
        var paths = [];
        this._paths.peach((function(path){
        ...
        this.propertiesChanged.peach(function(pair){
            pair.value = true;
        });
    ...
    addMarkers: function(defs){
        this._markers.peach(function(marker){
    ...
    serialize: function(){
        ...
        //add dockers triple
        var value = "";
        this._dockersByPath.peach((function(pair){
            pair.value.peach(function(docker){
...
/*            result.push({
    ...
    deserialize: function(data){
        ...
        if (oryxDockers) {
            ...
            dataByPath.peach((function(data, index){
	...
	layoutEdges : function(node, allEdges, offset){		
		...
		var Command = ORYX.Core.Command.extend({
			...
			execute: function(){
				...
				this.edges.peach(function(edge, i){
			...
			executeAgain: function(){
				this.changes.peach(function(change){
					// Reset the dockers
					this.removeAllDocker(change.edge);
					change.dockerPositions.peach(function(pos, i){	
						if (i==0||i==change.dockerPositions.length-1){ return }					
...
//    },
    ...
    editPaste: function(){
        ...
        canvas.eachChild(function(shape, parent){
			...
			shape.outgoing.peach(function(out){
...
ORYX.Plugins.Edit.DeleteCommand = ORYX.Core.Command.extend({
    ...
    execute: function(){
        this.shapesAsJson.peach(function(shapeAsJson){
            // Delete shape
...
ORYX.Plugins.View = {
	...
	construct: function(facade, ownPluginData) {
		...
		if (ownPluginData !== undefined && ownPluginData !== null) {
			ownPluginData.properties.peach( function(property) {			
    ...
    resize: function( position, shrink ){
    	...
    	resizeCanvas = function(position, extentionSize, facade) {
    		...
    		if( position == "N" || position == "W"){
    			...
    			dockers.peach(function(docker){ docker.bounds.moveBy(move)})
    		} else if( position == "S" ){
	...
	actOnDBLClick: function(evt, shape){
		...
		if( !nearestLabel ){
			...
			if( !nearestLabel ){
				...
				evtCoord.y += $Q("editor-header").clientHeight - $Q("canvasSection").scrollTop - 5;
				if (KISBPM.HEADER_CONFIG.showAppTitle == false)
				{
					evtCoord.y += 61;
				}
				
				evtCoord.x -= $Q("canvasSection").scrollLeft;
	...
	alignShapes: function(way) {
		...
		var commandClass = ORYX.Core.Command.extend({
			...
			execute: function(){
				...
				this.elements.peach(function(shape, index) {
...
ORYX.Plugins.Save = Clazz.extend({
    ...
    construct: function(facade){
		...
		// --> Execute
		//2018/06/05 this.facade.registerOnEvent(ORYX.CONFIG.EVENT_UNDO_EXECUTE, function(){ this.changeDifference++; this.updateTitle(); }.bind(this) );
		//2018/06/05 this.facade.registerOnEvent(ORYX.CONFIG.EVENT_EXECUTE_COMMANDS, function(){ this.changeDifference++; this.updateTitle(); }.bind(this) );
		// --> Saved from other places in the editor
		//2018/06/05 this.facade.registerOnEvent(ORYX.CONFIG.EVENT_SAVED, function(){ this.changeDifference = 0; this.updateTitle(); }.bind(this) );
		
		// --> Rollback
		//2018/06/05this.facade.registerOnEvent(ORYX.CONFIG.EVENT_UNDO_ROLLBACK, function(){ this.changeDifference--; this.updateTitle(); }.bind(this) );
		
		//TODO very critical for load time performance!!!
		//this.serializedDOM = DataManager.__persistDOM(this.facade);
		
		//2018/06/05 this.hasChanges = this._hasChanges.bind(this);
	},
	...
	handleMouseUp: function(event) {
		...
		if(this.dragEnable) {
			...
			if(!this.dragIntialized) {
				...
				//this.currentShapes.peach(function(shape) {shape.update()})
...
//		this._undockedEdgesCommand.dockers.peach(function(el){
...
//	},
		...
		if(this.isResizing) {
			...
			var commandClass = ORYX.Core.Command.extend({
				...
				update:function(offset){
					this.shape.getLabels().peach(function(label) {
						label.changed();
					});
	...
	beforeDrag: function(){
		...
		var undockEdgeCommand = ORYX.Core.Command.extend({
			...
			execute: function(){
				this.dockers.peach(function(el){
	...
	hideAllLabels: function(shape) {
			...
			shape.getLabels().peach(function(label) {
				label.hide();
			});
			// Hide all labels from docked shapes
			shape.getAllDockedShapes().peach(function(dockedShape) {
			...
			shape.getChildren().peach((function(value) {
	...
	showAllLabels: function(shape) {
			...
			// Do this recursive
			//shape.children.peach((function(value) {
...
//				} 
			...
			this.toMoveShapes.peach(function(value) {
				var shape = value;
				if(value instanceof ORYX.Core.Controls.Docker) {
					/* Get the Shape */
					shape = value.parent;
			...
			if(ORYX.CONFIG.GRID_ENABLED) {
				...
				this.distPointTimeout = window.setTimeout(function(){
					...
					distShapes.peach((function(value) {
	...
	snapToGrid: function(position) {
		...
		this.distPoints.peach(function(value) {
...
ORYX.Plugins.SelectedRect = Clazz.extend({
	...
	construct: function(parentId) {
		...
		this.node = ORYX.Editor.graft("http://www.w3.org/2000/svg", $Q(parentId),
					['g']);
...
ORYX.Plugins.Resizer = Clazz.extend({
	...
	construct: function(parentId, orientation, facade) {
		...
		this.node = ORYX.Editor.graft("http://www.w3.org/1999/xhtml", $Q('canvasSection'),
	...
	handleMouseDown: function(event) {
		...
		this.resizeStartCallbacks.peach((function(value) {
			value(this.bounds);
		}).bind(this));
	...
	handleMouseUp: function(event) {
		...
		this.resizeEndCallbacks.peach((function(value) {
			value(this.bounds);
		}).bind(this));
	...
	handleMouseMove: function(event) {
		...
		this.resizeCallbacks.peach((function(value) {
			value(this.bounds);
		}).bind(this));
	...
	move:function(offset, doLayout){
		...
		for(var i=0; i<this.moveShapes.length ;i++){
			...
			if (value instanceof ORYX.Core.Node) {
				...
				(value.dockers||[]).peach(function(d){
...
ORYX.Plugins.DragDocker = Clazz.extend({
	...
	handleMouseOut: function(event, uiObj) {
		...
		if(!this.docker && uiObj instanceof ORYX.Core.Controls.Docker) {
			...
			uiObj.dockers.peach(function(docker){
				docker.hide();
			});
		}
	},
	...
	handleMouseDown: function(event, uiObj) {
		...
		if(uiObj instanceof ORYX.Core.Controls.Docker && uiObj.isMovable) {
			...
			this.docker.parent.getLabels().peach(function(label) {
				label.hide();
			});
	...
	dockerMoved: function(event) {
		...
		if(!(event.shiftKey || event.ctrlKey) && !snapToMagnet) {
			...
			if (this.docker.parent) {
				...
				this.docker.parent.dockers.peach((function(docker){
	...
	dockerMovedFinished: function(event) {
		...
		this.dockerParent.getLabels().peach(function(label){
			label.show();
			//label.update();
		});
		...
		if(this.outerDockerNotMoved) {
			...
			var dragDockerCommand = ORYX.Core.Command.extend({
				...
				rollback: function(){
					...
					(this.removedDockers||$H({})).peach(function(d){
	...
	showMagnets: function(uiObj){
		uiObj.magnets.peach(function(magnet) {
			magnet.show();
		});
	},
...
ORYX.Plugins.ShapeHighlighting = Clazz.extend({
	...
	getPathByElements: function(elements){
		...
		elements.peach((function(element) {
			if(!element) {return}
...
ORYX.Plugins.Overlay = Clazz.extend({
	...
	show: function( options ){
		...
		if( options.attributes ){
			...
			// FOR EACH - Shape
			options.shapes.peach(function(el){
		...
		if ( options.node && isSVG) {
			...
			// FOR EACH - Node
			options.shapes.peach(function(el, index){
	...
	hide: function( options ){
		...
		// FOR EACH - Shape
		this.changes[options.id].peach(function(option){
			
			option.shapes.peach(function(el, index){
			...
			if( option._temps ){
				...
				option._temps.peach(function(tmp){
	...
	setAttributes: function( node, attributes ) {
		...
		childs.peach(function(e){ ids.push( $A(e.attributes).findAll(function(attr){ return attr.nodeValue.startsWith('url(#')}) )})
	...
	getAllChilds: function( node ){
		...
		$A(node.childNodes).peach(function( e ){ 
...
new function(){
	...
	ORYX.Plugins.BPMN2_0 = {
         ...
         */
        afterLoad: function(){
                this.facade.getCanvas().getChildNodes().peach(function(shape){
                        ...
                        if (this.hashedPositions[pool.id] && this.hashedPositions[pool.id].keys().any(function(key, i){
                                ...
                                var LanesHasBeenReordered = ORYX.Core.Command.extend({
                                        ...
                                        execute: function(){
                                                if (!this.executed){
                                                        this.executed = true;
                                                        this.lanes.peach(function(lane){
        ...
        adjustLanes: function(pool, lanes, x, y, scale){
                ...
                lanes.peach(function(l){
                        l.getChildNodes().peach(function(child){
        ...
        getAllExcludedLanes: function(parent, lane){
                var lanes = [];
                parent.getChildNodes().peach(function(shape){
        ...
        updateDepth: function(lane, fromDepth, toDepth){
                ...
                lane.getChildNodes().peach(function(shape){
	...
	var ResizeLanesCommand = ORYX.Core.Command.extend({
        ...
        construct: function(shape, parent, pool, plugin) {
                 ...
                 */
                this.shape.getChildShapes().peach(function(childShape) {
        ...
        execute: function() {
                ...
                if(this.lane) {                
                        ...
                        if (laUpL.y >= shUpL.y) {                              
                                this.lane.getChildShapes().peach(function(childShape) {
                                ...
                                this.shapeChildren.peach(function(shapeChild) {
                ...
                this.plugin.getLanes(this.parent).peach(function(childLane){
        ...
        rollback: function() {
                ...
                this.changes.peach(function(pair) {
