...
(function (yufp, window, factory) {
    ...
    function Lookup() {
        var _options = {
            lookupMgr: {},      //内存字典对象
            remoteUrl: backend.consoleService+'/api/s/dic', //远程URL
            remoteParamName: 'opttype', //远程参数名
            codeKey: 'enname',     //对应后台字段key
            codeValue: 'cnname', //对应后台字段value
    ...
    Lookup.prototype.loadRemote = function (types, callback) {
        var me = this;
        //TODO 暂且未考虑字典请求中队列
        me.forceLoad(types, function (rows) {
            var typeArr = types.split(',');
            for (var i=0,len = typeArr.length;i<len;i++){
                var type = typeArr[i];
                if (!me.limit) {
                    me.lookupMgr[type] = rows[type] || [];
                } else {
                    if(rows[type].length < me.limitLength){
                        me.lookupMgr[type] = rows[type];
                    } else {
                        me.storagePut(me.prefix + type, rows[type]);
                    }
                }
            }
        });
    ...
    Lookup.prototype.forceLoad = function (types, callback) {
        ...
        yufp.service.request({
            ...
            callback: function (code, msg, response) {
                if (code == 0 && response && response.code == 0 && yufp.type(callback) == 'function'){
     ...
     * 
     * 修改于2018/5/10
     * 添加bindData方法，主要解决当更改了codeKey,codeValue时，
     * 将codeKey,codeValue更改为key, value以配合select插件
     */
    Lookup.prototype.bind = function (type, callback) {
        ...
        var bindData = function(data) {
        	if(me.codeKey != 'key' && me.codeValue != 'value') {
        		var dicData = [];
        		for (var k=0; k<data.length; k++) {
                    dicData.push({key: data[k][me.codeKey], value: data[k][me.codeValue]});
                }
                callback.call(this, dicData);
        	} else {
