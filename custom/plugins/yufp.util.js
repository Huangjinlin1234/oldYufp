...
(function (yufp, window, factory) {
    ...
    Utils.prototype.exportExcelByTable = function (options) {
        ...
        if (typeof navigator !== "undefined" && /MSIE [1-9]\./.test(navigator.userAgent)) {
            //下载
        	var wopts = {bookType: 'xlsx', bookSST: true, type: 'base64'};//这里的数据是用来定义导出的格式类型
            var downloadifyAs = function(obj, filename) {
                //var button = document.createElement('button');
            	var createDownloadify = function(pid) {
            		var dify = Downloadify.create(pid,{
                        swf: './libs/downloadify/downloadify.swf',
                        downloadImage: './libs/downloadify/download.png',
                        width: 100,
                        height: 30,
                        filename: options.fileName + '.' + (wopts.bookType == 'biff2' ? 'xls' : wopts.bookType), 
            	...
            	if(options.param.downloadifyCallBack) {
                    options.param.downloadifyCallBack(createDownloadify);
                }
            }
            downloadifyAs(XLSX.write(wb, wopts));
        } else {
            ...
            var saveAs = function (obj, fileName) {//当然可以自定义简单的下载文件实现方式
                if('msSaveOrOpenBlob' in window.navigator){
                    window.navigator.msSaveOrOpenBlob(obj, fileName || '下载');
                } else {
     ...
     * @returns {*}
     */
    Utils.prototype.exportCsvByTable = function (options) {
        var table_ref = options.ref;
        var colums = table_ref.tableColumns;
        var colums_ = new Array();
        var tableColumns = colums.concat([]);
        ...
        if (typeof navigator !== "undefined" && /MSIE [1-9]\./.test(navigator.userAgent)) {
            //下载
            var wopts = {bookType: 'csv', bookSST: true, type: 'base64'};//这里的数据是用来定义导出的格式类型
            var downloadifyAs = function(obj, filename) {
                //var button = document.createElement('button');
                var createDownloadify = function(pid) {
                    var dify = Downloadify.create(pid,{
            ...
            var wopts = {bookType: 'csv', bookSST: true, type: 'binary'};//这里的数据是用来定义导出的格式类型
            var saveAs = function (obj, fileName) {//当然可以自定义简单的下载文件实现方式
                if('msSaveOrOpenBlob' in window.navigator){
                    window.navigator.msSaveOrOpenBlob(obj, fileName || '下载');
                } else {
     ...
     * 对象拷贝赋值
     * 修改于2018/5/9
     * 该方法用于从源对象中将目标对象中有的属性（key）赋值到目标对象中，没有的则不赋值
     */
    Utils.prototype.copyObj = function (dest, src) {
...
//      num = num > 0 && num <= 20 ? num : 2;
    ...
    /**
     * @created by wanglh4
     * @updated by 2019/11/15
     * @description 为实际业务需求，小数数值百分比显示
     */
    Utils.prototype.decimalToPercent = function (money, num) {
        /*
          * 参数说明：
          * money：要格式化的数字
          * num：保留几位小数
        ...
        if (String(money).indexOf('%') != -1) {
            money = parseFloat(String(money).replace('%','')) / 100;
        }
//      num = num > 0 && num <= 20 ? num : 2;
        if (money != null && money != '')
