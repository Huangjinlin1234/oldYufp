            ...
            data: function () {
                ...
                return {
                    ...
                    queryFields: [
                        ...
                        { placeholder: '借款人证件类型', field: 'certType', type: 'select', dataCode: 'STD_ZB_CERT_TYP', datacodeFilter: function(options){
                                this.typeOptions = [];
                        ...
                        { placeholder: '借款人证件号码', field: 'certCode', type: 'input' },
                        { placeholder: '借款人姓名', field: 'cusName', type: 'input' }
                        ...
                        //         this.typeOptions = [];
                        //         for(var i=0; i<options.length; i++){
                        //             if(options[i].key != '101' && options[i].key != '202' && options[i].key != 'FB99' ){
                        //                 this.typeOptions.push(options[i]);
                        //             }
                        //         }
                        //     }}
                    ...
                    queryButtons: [
                        { label: '查询',op: 'submit',type: 'primary',icon: 'search',
                            ...
                            //有查询条件时
                            if(valid){
                                if((null==model.applyDateMin || model.applyDateMin == "" ) && (null==model.applyDateMax || model.applyDateMax == "" )){
                                    //同时为空不做处理
                                    _self.$refs.reftable.remoteData(model);
                                }else{
                                    if((null==model.applyDateMin || model.applyDateMin == "" ) || (null==model.applyDateMax || model.applyDateMax == "" )){
                                        //其中一个为空，提示
                                        _self.$message('开始日期或结束日期必须同时作为查询条件!');
                                    }else{
                                        //都有值时
                                        var dateMin = new Date(model.applyDateMin);
                                        var dateMax = new Date(model.applyDateMax);
                                        if(dateMin > dateMax){
                                            _self.$message('结束日期不能小于开始日期!!');
                                        }else {
                                            _self.$refs.reftable.remoteData(model);
                                        }
                                    }
                    ...
                    tableColumns: [
                        ...
                        { label: '借款人客户号', prop: 'cusId', sortable: true, resizable: true },
                        { label: '借款人姓名', prop: 'cusName', sortable: true, resizable: true },
                        { label: '借款人证件类型', prop: 'certType', sortable: true, resizable: true, dataCode: 'STD_ZB_CERT_TYP'  },
                        { label: '借款人证件号码', prop: 'certCode', sortable: true, resizable: true },
                        { label: '借款人联系电话', prop: 'mobile', sortable: true, resizable: true },
                        { label: '凭证上传日期', prop: 'startDate', sortable: true, resizable: true },
                        {
                            label: '凭证查看', sortable: true, resizable: true ,
                            template: function () {
                                return '<template scope="scope">\
                    			<a href="javascript:void(0);" style="text-decoration:underline;"@click="_$event(\'custom-row-click\', scope.row)">点击查看</a>\
                    		</template>';
                            }
                        },
                    ...
                    tableColumnsImages: [
                        ...
                        { label: '上传时间', prop: 'lastModifyTime', sortable: true, resizable: true }
                    ],
