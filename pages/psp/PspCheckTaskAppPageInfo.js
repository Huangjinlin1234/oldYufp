...
//                            Number(num * 100).toFixed(4) + '%';
            ...
            updateFields: [{
            	columnCount: 3,
              fields: [
                	 { field: 'bizSerno', label: '检查任务号',disabled:true},
...
//                	       { required: true, message: '请输入检查名称', trigger: 'blur' },
                   ...
                   { field: 'legalOrgName', label: '法人机构名称',value:yufp.session.legalOrg.orgName,type:'input',hidden:true},
                                ...
                                yufp.service.request({
                                    ...
                                    callback: function (code, message, response) {
                                      if (code == 0) {
                                        ...
                                        _self.$refs.form2.resetFn();
                                        yufp.util.copyObj(_self.$refs.form2.formModel, contInfo);
                                         yufp.util.copyObj(_self.$refs.form2.formModel, response.data);
                                        _self.$message('操作成功');
                                      } else {
                                        _self.$message('操作失败');
                                      }
...
//                      rules:[{validator: yufp.validator.IDCard, message: '请输入正确的身份证号', trigger: 'blur'}
//                                    {max: 18, message: '最大长度为18', required: true} ]
                   },
          ...
          updateFields3: [{
                   columnCount: 1,
              fields: [
                   { field: 'othUnFactor', label: '',type:"textarea",rules:[
            ...
            updateFields4: [{
               columnCount: 1,
          fields: [
               { field: 'expSugg', label: '',type:"textarea",rules:[
                   {min: 0,  max: 1000, message:'长度不能大于1000',  trigger: 'blur'}
                   ]}
                ]
                }],
              warnFields: [{
                   columnCount: 1,
              fields: [
                   { field: 'warnInfo', label: '规则反馈信息',type:"textarea",rules:[
        ...
        methods: {
          ...
          */
          ...
          addFn: function () {
            var _self = this;
//                _self.$confirm('是否确认新增？','提示',{type:'warning'}).then(function(){ 
                 yufp.service.request({
                    method: 'POST',
                    data: {
                       createUser:yufp.session.userId,
                        createTime:yufp.session.OPENDAY,
                        legalOrgCode:yufp.session.legalOrg.orgCode,
                        legalOrgName:yufp.session.legalOrg.orgName,
                        inputBrId:yufp.session.org.orgCode,
                        orgCode:yufp.session.org.orgCode,
                        mainBrId:yufp.session.org.orgCode,
                        orgName:yufp.session.org.orgName
                    },
                        ...
                        if (response.code == 0) {
                            _self.switchStatus('EDIT', true);
                             ...
                             _self.$nextTick(function () {
                                    _self.$refs.form1.resetFn();
                                    _self.$refs.form2.resetFn();
                                    _self.$refs.form3.resetFn();
                                    _self.$refs.form4.resetFn();
                                    _self.wflag = false;
                                    _self.$message('新增任务成功,跳转到修改页面');
                                    yufp.util.copyObj(_self.$refs.form1.formModel, response.data);
...
//          }).catch(function(){});
          ...
          deleteFn: function () {
            ...
            if (selections.length < 1) {
             ...
             var chkState=obj.chkState;
             var chkType=obj.chkType;
             if(chkState!='01' ||  chkType!='03'){
                this.$message({ message: '只有检查状态为待登记的不定期任务才能删除!', type: 'warning' });
                return;
             }
