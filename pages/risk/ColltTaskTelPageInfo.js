 ...
 * @description 催收任务流转申请表
 */
define(function (require, exports) {

    // page加载完成后调用ready方法
    exports.ready = function (hashCode, data, cite) {
        yufp.lookup.reg('TO_DO,STD_COLLT_WAY,IDENT_WAY,STD_RISK_LEVEL,CRUD_TYPE,STD_RISK_LEVEL,COLLT_TASK_STATUS,STD_COLLT_WAY,IDENT_WAY,STD_ZB_CERT_TYP,APPLY_CHANNEL_TYPE,STD_ZB_ACC_STATUS,STD_ZX_CUR_TYPE,STD_ZB_TERM_TYPE,STD_ZB_IR_ADJ_MODE,STD_ZB_ASSURE_MEANS,STD_PRD_REPAY_MODE,STD_ZB_EFR_CHNG_IND');
        var vm = yufp.custom.vue({
            ...
            data: function () {
                ...
                return {
                    ...
                    // 首页：催收任务信息表
                    taskInfoDataUrl: backend.riskmService + '/api/collt/task/tel/infos',
                    //任务认领
                    taskInfoDataUrl2: backend.riskmService + '/api/collt/task/infos/call',
                    // 催收记录
                    ...
                    tableColumns: [
                        ...
                        { label: '风险类型', prop: 'riskType', sortable: true, resizable: true ,type:'select',dataCode:'STD_RISK_LEVEL'},
                    ...
                    cusInfoFields:[{
                        ...
                        fields: [
                            ...
                            { field: 'cusType', label: '客户类型' ,disabled: true ,type: 'select',dataCode: 'STD_ZB_CUS_TYPE'},
                    ...
                    accLoanInfoFields:[
                        {columnCount: 2,
                            fields: [
                                ...
                                { field: 'delayIntCumu1', label: '逾期利息金额',disabled: true },
                                { field: 'overDays', label: '逾期利息天数',disabled: true },
                                ...
                                { field: 'intRateType', label: '利率类型',disabled: true, dataCode: 'STD_ZB_EFR_CHNG_IND', type: 'select', hidden: true},
                    ...
                    colltRcdTableColumns:[
                        ...
                        { label: '催收记录流水号', prop: 'colltSerno', sortable: true, resizable: true ,hidden:'true' },
                        { label: '保证金金额', prop: 'marginAmt', sortable: true, resizable: true ,hidden:'true' },
                        { label: '代理开始日期', prop: 'agentStartDt', sortable: true, resizable: true ,hidden:'true' },
                        { label: '代理结束日期', prop: 'agentEndDt', sortable: true, resizable: true ,hidden:'true' },
                        { label: '催收回收率', prop: 'colltRecRate', sortable: true, resizable: true,hidden:'true'  },
                        { label: '登记日期', prop: 'inputDate', sortable: true, resizable: true ,hidden:'true' },
                        { label: '最新修改人', prop: 'lastChgUsr', sortable: true, resizable: true ,hidden:'true' },
                        { label: '最后修改时间', prop: 'lastChgTime', sortable: true, resizable: true,hidden:'true'  }
                    ],
                    ...
                    colltRegisterInfoFields:[{
                        ...
                        fields: [
                            ...
                            { field: 'colltCode', label: '催收代码',rules: [{required: true, message: '必填项', trigger: 'blur'}]},
                            ...
                            { field: 'repayDate', label: '还款日期',type: 'date' ,editable:false,rules: [{required: true, message: '必填项'}]},
                            { field: 'colltRestRemark', label: '催收结果备注',type:'textarea',rules: [{required: true, message: '必填项'}]},
                            ...
                            { field: 'flowColltTaskType', label: '流转催收任务类型',type:'select',dataCode:'STD_COLLT_WAY',
                                datacodeFilter: function(options){
                                    ...
                                    for(var i=0; i<options.length; i++){
                                        if(options[i].key == '02' || options[i].key == '03' || options[i].key == '04' || options[i].key == '05'){
                                            this.typeOptions.push(options[i]);
                                        }
                                    }
                                },
                                rules: [{required: true, message: '必填项'}]},
                            { field: 'flowReson', label: '流转原因',type:'textarea',rules: [{required: true, message: '必填项'}]},
                    ...
                    updateButtons: [
                        ...
                        { label: '保存', type: 'primary', icon: 'check', hidden: false, click: function (model) {
                                ...
                                yufp.service.request({
                                    method: rMethod,
                                    url: '/api/collt/task/flow/app',
                                    data: model,
                                    callback: function (code, message, response) {
                                        if (code == 0) {
                                            _self.$refsreftable.remoteData();
                                            _self.$message('操作成功');
                                            _self.dialogVisible = false;
                                        } else {
                                            _self.$message('操作失败');
            ...
            methods: {
                ...
                taskReg: function () {
                    ...
                    this.$nextTick(function () {
                        ...
                        /*  _self.$refs.colltRegisterInfoRef.formModel.colltTaskNo = obj.colltTaskNo;
                          _self.$refs.colltRegisterInfoRef.formModel.colltWay = obj.colltWay;*/
                ...
                historyInfo: function () {
                    var _self = this;
                    if (this.$refs.reftable.selections.length != 1) {
                        this.$message({ message: '请先选择一条记录', type:'success' });
                        return;
                    }
                    _self.taskGetdialogVisible = false;
                    _self.messageDialogVisible = false;
                ...
                commitFlowInfoFn: function(){
                    ...
                    yufp.service.request({
                        method: 'POST',
                        url: backend.flowService + "/api/collt/task/flow/app/commit",
                        data: obj,
                        callback: function (code, message, response) {
                            if (response.code==0) {
                                _self.dialogVisible = false;
                ...
                sureFn: function(){
                    ...
                    yufp.service.request({
                        method: 'POST',
                        url: backend.flowService + "/api/collt/task/tel/sms",
                        data: obj,
                        callback: function (code, message, response) {
