        ...
        yufp.custom.vue({
            ...
            data: function () {
                ...
                return {
                    ...
                    queryFields: [
                        {placeholder: '律师事务所编号', field: 'outsOrgCode', type: 'input'},
                        {placeholder: '律师事务所名称', field: 'outsOrgName', type: 'input'},
                    ...
                    ratebaseParams: { cooprOrgType:'003' },
                    queryOrgVerFields: [
                    ...
                    /**分配查看 start*/
                    dataUrlDistri: backend.riskmService + "/api/collt/task/law/infos",
                    distriParams: {},
                    queryDistriFields: [
                    ...
                    cusBaseInfoFields: [{
                        columnCount: 2,
                        fields: [
                            {field: 'outsOrgCode', label: '律师事务所编号', disabled: true, hidden: false},
                            {field: 'outsOrgName', label: '律师事务所名称', disabled: true, hidden: false},
                    ...
                    litiCollInfoFields: [
                        {
                            ...
                            fields: [
                                ...
                                {field: 'colltCode', label: '催收代码', disabled: false, hidden: false, rules: [{required: true, message: '必填项', trigger: 'blur'}]},
                                {field: 'colltRecRate', label: '催收回款率(%)', disabled: true, hidden: false, rules: [{required: true, message: '必填项', trigger: 'blur'}]},
                    ...
                    flowColumns: [
                        ...
                        {label: '操作员', prop: 'aprvUserCode', sortable: true, resizable: true},
                    ...
                    flowInfoFields: [
                        {
                            ...
                            fields: [
                                {field: 'flowColltTaskType', label: '流转催收任务类型', disabled: false,type:'select',dataCode:'STD_COLLT_WAY',
                                    datacodeFilter: function (options) {
                                        ...
                                        for (var i = 0; i < options.length; i++) {
                                            if (options[i].key == '02' || options[i].key == '03' || options[i].key == '05') {
                                ...
                                {field: 'createUser',label: '操作员', hidden: true },
                                {field: 'createTime', label: '操作时间', hidden: true},
                                {field: 'createUserOrg',label:'操作员所属机构', hidden: true}
                            ]
                        },
                        {
                            columnCount: 1,
                            fields: [
                                {
                                    field: 'flowReson', label: '流转原因', type: 'textarea', rows: 3,
                                    rules: [{required: true, message: '必填项', trigger: 'blur'}]
                                }
                            ]
                        }
                    ...
                    flowInfoButtons: [
                        {
                            label: '提交', type: 'primary', icon: 'check', hidden: false, click: function (model) {
                                ...
                                yufp.service.request({
                                    method: 'GET',
                                    url: backend.riskmService + "/api/collt/task/flow/info/" + obj.colltTaskNo,
                                    data:{},
                                    callback: function (code, message, response) {
                                        if (code == 0) {
                                            if (response.rows > 0) {
                                                _self.$message({message: '该催收任务存在“正常”的流转申请，暂无法再次发起申请！', type: 'warning'});
                                            } else {
                                                //保存信息至催收任务流转申请表
                                                model.taskSts = obj.taskSts;
                                                yufp.service.request({
                                                    method: 'POST',
                                                    url: backend.riskmService + '/api/collt/litiTask/save',
                                                    data: model,
                                                    callback: function (code, message, response) {
                                                        if (code == 0) {
                                                            var params = {
                                                                flowApplyNo:response.message
                                                            };
                                                            yufp.service.request({
                                                                method: 'GET',
                                                                url: backend.flowService + '/api/collt/litiTask/flow/commit',
                                                                data: params,
                                                                callback: function (code, message, response) {
                                                                    if (response.code == 0) {
                                                                        ...
                                                                        _self.$message({message: '流程发起失败', type: 'warning'});
                                                                    }
                                                                }
                                                            });
                                                        } else {
                                                            _self.$message({message: '保存失败', type: 'warning'});
                                                        }
                                                    }
                                                });
                                            }
                     ...
                     singleFields: [
                        {
                            ...
                            fields: [
                                ...
                                {field: 'outsOrgCode', label: '律师事务所编号', disabled: false,  hidden: true},
                                {field: 'outsOrgName', label: '律师事务所名称', disabled: false,  hidden: true}
                            ]
                        },
                        {
                            columnCount: 1,
            ...
            methods: {
                ...
                exportFn: function () {
                    ...
                    this.$confirm('是否导出数据?', '提示').then(function () {
                        yufp.util.exportExcelByTable({
                            fileName: "诉讼催收任务分配信息_"+obj.outsOrgName+'_'+currDt,
                            importType: 'service', // page当前页 selected 选中的数据  service 后端数据
                            ref: _self.$refs.reftableDistri,
                            url: backend.riskmService + "/api/collt/task/law/infos",
                            method: 'POST',
                            param: {
                ...
                manualFn: function () {
                    ...
                    if (obj[0].taskSts != "01" && obj[0].taskSts != "07") {
                        _self.$message({message: '现阶段只支持对任务状态为"正常"、"处理中"的催收任务进行人工流转！', type: 'warning'});
                        return;
                    }
                    //
                    _self.dialogSonVisible = true;
                            ...
                            if (code == 0) {
                                _self.$nextTick(function () {
                                    ...
                                    yufp.extend(_self.$refs.litiCollInfo.formModel,
                                        ...
                                        {colltRecRate:response.rows.colltRecRate},
                ...
                manualFlow: function (item) {//collt_task_flow_app
                    ...
                    this.$nextTick(function () {
                        ...
                        _self.$refs.flowInfo.resetFn();
                        yufp.extend(_self.$refs.flowInfo.formModel,
                            {colltTaskNo: item.colltTaskNo},
                            {opUserCode:item.opUserCode},
